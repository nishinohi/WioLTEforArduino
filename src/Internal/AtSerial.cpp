...
bool AtSerial::ReadResponseInternal(const char *pattern, unsigned long timeout,
                                    std::string *response, int responseMaxLength) {
    DEBUG_PRINT("-> ");
...
bool AtSerial::ReadResponse(const char *pattern, unsigned long timeout, std::string *capture) {
    ...
    while (true) {
        ...
        std::string response;
        if (!ReadResponseInternal(internalPattern, READ_BYTE_TIMEOUT, &response,
                                  RESPONSE_MAX_LENGTH))
            return false;
...
bool AtSerial::WriteCommandAndReadResponse(const char *command, const char *pattern,
                                           unsigned long timeout, std::string *capture) {
