...
static int is_quantifier(const char *re) { return re[0] == '*' || re[0] == '+' || re[0] == '?'; }

static int toi(int x) { return isdigit(x) ? x - '0' : x - 'W'; }

static int hextoi(const unsigned char *s) { return (toi(tolower(s[0])) << 4) | toi(tolower(s[1])); }
...
static int bar(const char *re, int re_len, const char *s, int s_len, struct regex_info *info,
               int bi) {
            ...
            if (re[i + step] == '?') {
                ...
                do {
                    ...
                    if (ni >= re_len) {
                        ...
                        nj = j2;
                    } else if ((n2 = bar(re + ni, re_len - ni, s + j2, s_len - j2, info, bi)) >=
                               0) {
            ...
            if (re_len - (i + step) <= 0) {
                ...
                for (j2 = 0; j2 <= s_len - j; j2++) {
                    if ((n = doh(s + j, s_len - (j + j2), info, bi)) >= 0 &&
                        bar(re + i + step, re_len - (i + step), s + j + n, s_len - (j + n), info,
                            bi) >= 0)
                        break;
                }
            }
    ...
    for (i = 0; i < re_len; i += step) {
        ...
        if (re[i] == '|') {
            ...
            if (re[i + 1] == 'x') {
                ...
                FAIL_IF(re[i + 1] == 'x' && !(isxdigit(re[i + 2]) && isxdigit(re[i + 3])),
