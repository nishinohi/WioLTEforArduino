...
#define LINEAR_SCALE(val, inMin, inMax, outMin, outMax)                                            \
...
static bool SplitUrl(const char *url, const char **host, int *hostLength, const char **uri,
                     int *uriLength) {
...
int WioLTE::GetFirstIndexOfReceivedSMS() {
    ...
    if (!_AtSerial.WriteCommandAndReadResponse("AT+CMGF=0", "^OK$", 500, NULL))
        return -1;
    ...
    int messageIndex = -1;
    while (true) {
        if (!_AtSerial.ReadResponse("^(OK|\\+CMGL: .*)$", 500, &response))
            return -1;
        if (response == "OK")
            break;
        if (messageIndex < 0) {
...
void WioLTE::PowerSupplyCellular(bool on) { PowerSupplyLTE(on); }
...
bool WioLTE::TurnOnOrReset() {
    ...
    if (!_AtSerial.WriteCommandAndReadResponse("ATE0", "^OK$", 500, NULL))
        return RET_ERR(false, E_UNKNOWN);
    if (!_AtSerial.WriteCommandAndReadResponse("AT+QURCCFG=\"urcport\",\"uart1\"", "^OK$", 500,
                                               NULL))
        return RET_ERR(false, E_UNKNOWN);
    if (!_AtSerial.WriteCommandAndReadResponse("AT+QSCLK=1", "^(OK|ERROR)$", 500, NULL))
        return RET_ERR(false, E_UNKNOWN);

    // TODO ReadResponseCallback
    // if (!_AtSerial.WriteCommandAndReadResponse("AT+CGREG=2", "^OK$", 500, NULL)) return
    // RET_ERR(false, E_UNKNOWN); if (!_AtSerial.WriteCommandAndReadResponse("AT+CEREG=2", "^OK$",
    // 500, NULL)) return RET_ERR(false, E_UNKNOWN); if
    // (!_AtSerial.WriteCommandAndReadResponse("AT+CGREG?", "^OK$", 500, NULL)) return
    // RET_ERR(false, E_UNKNOWN); if (!_AtSerial.WriteCommandAndReadResponse("AT+CEREG?", "^OK$",
    // 500, NULL)) return RET_ERR(false, E_UNKNOWN);

    if (!_AtSerial.ReadResponse("^\\+CPIN: READY$", 10000, NULL))
        return RET_ERR(false, E_UNKNOWN);
...
bool WioLTE::TurnOff(long timeout) {
    ...
    while (true) {
        ...
        if (!_AtSerial.ReadResponse("^(OK|ERROR)$", 500, &response))
            return RET_ERR(false, E_UNKNOWN);
        if (response == "OK")
            break;
...
int WioLTE::GetReceivedSignalStrength() {
    ...
    if (rssi == 0)
        ...
        return RET_OK(-115);
    else if (102 <= rssi && rssi <= 190)
        return RET_OK((int)LINEAR_SCALE((double)rssi, 102, 190, -114, -26));
    else if (rssi == 191)
        return RET_OK(-25);
    else if (rssi == 199)
        return RET_OK(-999);
...
bool WioLTE::GetTime(struct tm *tim) {
    ...
    if (!_AtSerial.ReadResponse("^\\+CCLK: (.*)$", 500, &response))
        return RET_ERR(false, E_UNKNOWN);
    if (!_AtSerial.ReadResponse("^OK$", 500, NULL))
        return RET_ERR(false, E_UNKNOWN);

    if (strlen(response.c_str()) != 22)
        return RET_ERR(false, E_UNKNOWN);
...
bool WioLTE::SendSMS(const char *dialNumber, const char *message) {
    if (!_AtSerial.WriteCommandAndReadResponse("AT+CMGF=1", "^OK$", 500, NULL))
        return RET_ERR(false, E_UNKNOWN);

    StringBuilder str;
    if (!str.WriteFormat("AT+CMGS=\"%s\"", dialNumber))
        return RET_ERR(false, E_UNKNOWN);
    _AtSerial.WriteCommand(str.GetString());
    if (!_AtSerial.ReadResponse("^> ", 500, NULL))
        return RET_ERR(false, E_UNKNOWN);
    ...
    if (!_AtSerial.ReadResponse("^OK$", 120000, NULL))
        return RET_ERR(false, E_UNKNOWN);
...
int WioLTE::ReceiveSMS(char *message, int messageSize, char *dialNumber, int dialNumberSize) {
    ...
    StringBuilder str;
    if (!str.WriteFormat("AT+CMGR=%d", messageIndex))
        return RET_ERR(-1, E_UNKNOWN);
    _AtSerial.WriteCommand(str.GetString());

    if (!_AtSerial.ReadResponse("^\\+CMGR: .*$", 500, NULL))
        return RET_ERR(-1, E_UNKNOWN);

    std::string response;
    if (!_AtSerial.ReadResponse("^(.*)$", 500, &response))
        return RET_ERR(-1, E_UNKNOWN);
    ...
    if (!ConvertHexToBytes(hex, data, dataSize))
        return RET_ERR(-1, E_UNKNOWN);
    ...
    if (tpMti >= dataEnd)
        return RET_ERR(-1, E_UNKNOWN);
    if ((*tpMti & 0xc0) != 0)
        return RET_ERR(-1, E_UNKNOWN); // SMS-DELIVER
    ...
    if (dialNumber != NULL && dialNumberSize >= 1) {
        if (!SmAddressFieldToString(tpOaSize, dialNumber, dialNumberSize))
            return RET_ERR(-1, E_UNKNOWN);
    }

    if (messageSize < *tpUdl + 1)
        return RET_ERR(-1, E_UNKNOWN);
...
bool WioLTE::DeleteReceivedSMS() {
    ...
    StringBuilder str;
    if (!str.WriteFormat("AT+CMGD=%d", messageIndex))
        return RET_ERR(false, E_UNKNOWN);
...
bool WioLTE::WaitForPSRegistration(long timeout) {
    ...
    while (true) {
        ...
        if (!_AtSerial.ReadResponse("^\\+CGREG: (.*)$", 500, &response))
            return RET_ERR(false, E_UNKNOWN);
        parser.Parse(response.c_str());
        if (parser.Size() < 2)
            return RET_ERR(false, E_UNKNOWN);
        ...
        if (!_AtSerial.ReadResponse("^\\+CEREG: (.*)$", 500, &response))
            return RET_ERR(false, E_UNKNOWN);
        parser.Parse(response.c_str());
        if (parser.Size() < 2)
            return RET_ERR(false, E_UNKNOWN);
...
bool WioLTE::Activate(const char *accessPointName, const char *userName, const char *password,
    ...
    if (!WaitForPSRegistration(0)) {
        StringBuilder str;
        if (!str.WriteFormat("AT+QICSGP=1,1,\"%s\",\"%s\",\"%s\",3", accessPointName, userName,
                             password))
            return RET_ERR(false, E_UNKNOWN);
        if (!_AtSerial.WriteCommandAndReadResponse(str.GetString(), "^OK$", 500, NULL))
            return RET_ERR(false, E_UNKNOWN);
...
bool WioLTE::SyncTime(const char *host) {
    StringBuilder str;
    if (!str.WriteFormat("AT+QNTP=1,\"%s\"", host))
        return RET_ERR(false, E_UNKNOWN);
    if (!_AtSerial.WriteCommandAndReadResponse(str.GetString(), "^OK$", 500, NULL))
        return RET_ERR(false, E_UNKNOWN);
    if (!_AtSerial.ReadResponse("^\\+QNTP: (.*)$", 125000, NULL))
        return RET_ERR(false, E_UNKNOWN);
...
bool WioLTE::GetLocation(double *longitude, double *latitude) {
    ...
    if (!_AtSerial.WriteCommandAndReadResponse("AT+QLOCCFG=\"contextid\",1", "^OK$", 500, NULL))
        return RET_ERR(false, E_UNKNOWN);

    _AtSerial.WriteCommand("AT+QCELLLOC");
    if (!_AtSerial.ReadResponse("^(\\+QCELLLOC: .*|\\+CME ERROR: .*)$", 60000, &response))
        return RET_ERR(false, E_UNKNOWN);
    if (strncmp(response.c_str(), "+QCELLLOC: ", 11) != 0)
        return RET_ERR(false, E_UNKNOWN);
...
int WioLTE::HttpGet(const char *url, char *data, int dataSize, const WioLTEHttpHeader &header,
    ...
    if (strncmp(url, "https:", 6) == 0) {
        if (!_AtSerial.WriteCommandAndReadResponse("AT+QHTTPCFG=\"sslctxid\",1", "^OK$", 500, NULL))
            return RET_ERR(-1, E_UNKNOWN);
        if (!_AtSerial.WriteCommandAndReadResponse("AT+QSSLCFG=\"sslversion\",1,4", "^OK$", 500,
                                                   NULL))
            return RET_ERR(-1, E_UNKNOWN);
        if (!_AtSerial.WriteCommandAndReadResponse("AT+QSSLCFG=\"ciphersuite\",1,0XFFFF", "^OK$",
                                                   500, NULL))
            return RET_ERR(-1, E_UNKNOWN);
        if (!_AtSerial.WriteCommandAndReadResponse("AT+QSSLCFG=\"seclevel\",1,0", "^OK$", 500,
                                                   NULL))
            return RET_ERR(-1, E_UNKNOWN);
    }

    if (!_AtSerial.WriteCommandAndReadResponse("AT+QHTTPCFG=\"requestheader\",1", "^OK$", 500,
                                               NULL))
        return RET_ERR(-1, E_UNKNOWN);
    ...
    StringBuilder str;
    if (!str.WriteFormat("AT+QHTTPGET=%d,%d", timeoutSec, headerSb.Length()))
        return RET_ERR(false, E_UNKNOWN);
    _AtSerial.WriteCommand(str.GetString());
    ...
    if (!_AtSerial.ReadResponse("^OK$", 1000, NULL))
        return RET_ERR(false, E_UNKNOWN);
    if (!_AtSerial.ReadResponse("^\\+QHTTPGET: (.*)$", (timeoutSec + 1) * 1000, &response))
        return RET_ERR(-1, E_UNKNOWN);
...
bool WioLTE::HttpPost(const char *url, const char *data, int *responseCode,
                      const WioLTEHttpHeader &header, long timeout) {
    std::string response;
    ArgumentParser parser;
    ...
    StringBuilder str;
    if (!str.WriteFormat("AT+QHTTPPOST=%d,%d,%d", headerSb.Length() + strlen(data), timeoutSec,
                         timeoutSec))
        return RET_ERR(false, E_UNKNOWN);
    _AtSerial.WriteCommand(str.GetString());
    if (!_AtSerial.ReadResponse("^CONNECT$", 60000, NULL))
    ...
    if (!_AtSerial.ReadResponse("^OK$", 1000, NULL))
        return RET_ERR(false, E_UNKNOWN);
    if (!_AtSerial.ReadResponse("^\\+QHTTPPOST: (.*)$", (timeoutSec + 1) * 1000, &response))
        return RET_ERR(false, E_UNKNOWN);
    parser.Parse(response.c_str());
    if (parser.Size() < 1)
        return RET_ERR(false, E_UNKNOWN);
...
void WioLTE::SystemReset() { NVIC_SystemReset(); }
